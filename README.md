...
更多健康检查、冒烟测试、备份、反向代理与升级说明见 [`DEPLOYMENT.md`](./DEPLOYMENT.md)。公网部署建议在前面再放一层 Nginx/Caddy 等 HTTPS 反向代理（请求体上限需至少放行 `1 GiB`，并保留 `Authorization`/`x-api-key` 头、支持长连接 SSE、关闭对流式路径的缓冲）。
...
- **出口代理池**
  ...
  - 上游 429 会先自动切换到其他可用节点重试一次；连续 5 次上游 429 自动禁用节点，后台每 10 分钟发起一次最小模型请求探测，成功后自动恢复；其他失败进入冷却（默认 5 分钟），冷却结束后也会自动探测，成功后清除异常
  - 每日请求上限与按日自动重置，可选“达上限自动禁用 / 次日自动恢复”
  - 节点连通性测试、成功/失败计数、总 Token 与今日 Token、最近 20 次请求结果（用于前端可视化），支持单节点清空统计
...
10. **释放与记账**：释放限流名额与在途登记，写入指标（HTTP/上游延迟分位、状态码、按代理分布）与请求日志（含出口代理字段）。

### 代理选择逻辑（优先填充，非轮询）

从所有“启用、未冷却、未达每日上限、未达并发上限”的节点中，**按权重从高到低排序，始终取第一个**。也就是说高权重节点会被持续打满，直到它不可用，才轮到下一个——而不是请求在节点间轮转。节点连续 5 次上游 429 自动禁用，后台每 10 分钟通过模型请求探测一次，收到 2xx 后自动重新启用；其他失败进入冷却（默认 5 分钟），冷却结束后后台同样会自动探测，成功后清除“异常”状态。手动禁用和每日上限禁用不会被后台自动开启。
...
- **设置**：上游超时、请求体限制（默认 `1 GiB`）、默认流式、代理使用模式、链式前置代理、文件日志与审计开关、日志保留天数
...
- 节点连续 5 次收到上游 429 会被自动禁用；后台每 10 分钟通过最小模型请求探测，收到 2xx 后自动重新启用。其他失败会进入冷却（默认 5 分钟），冷却结束后后台同样自动探测，收到 2xx 后清除“异常”状态。手动禁用和每日上限禁用不会被后台自动开启。
